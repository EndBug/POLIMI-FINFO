...
void scrivi(char path[]) {
  ...
  */
  ...
  /*
    ...
    L'operazione di apertura di un file non va sempre a buon fine, fopen restituisce 
    NULL se l'operazione fallisce.
  */
  if (fp != NULL) { /* <=> if (fp) */
    for (c = '0'; c <= '9'; c++)
      /* 
        ...
        Volendo si può utilizzare per scrivere direttamente a video, sullo stout.
      */
      fprintf(fp, "%c ", c);
